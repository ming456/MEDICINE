...
<caption>顾客信息</caption>
	...
	<tr align=center>
		...
		<td><%=rs.getString("csymptom")%></td>
		<td><%=rs.getString("mno")%></td>
		<td><%=rs.getString("ano")%></td>
		<td><%=rs.getString("cdate")%></td>
	    <td><%=rs.getString("cremark")%></td>
	</tr>
	<%
		rs.next();
	}%>
...
{%>
<td><a href=../display/display1.jsp?to_page=1>第一页</a></td>
<td><a href=../display/display1.jsp?to_page=<%=show_page-1%>>前一页</a></td>
<%}
if(show_page!=page_all) //如果到了最后一页，则在网页上不显示"后一页"和"最后一页"的超链接
{%>
<td><a href=../display/display1.jsp?to_page=<%=show_page+1%>>后一页</a></td>
<td><a href=../display/display1.jsp?to_page=<%=page_all%>>最后一页</a></td>
<%
}
%>
</tr>
